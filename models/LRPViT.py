...
def init_model(pretrained=True):
    ...
    model = vit_LRP(num_classes=2, pretrained=True)
    weights = "artifacts/vit_lrp2.pth"
    if pretrained:
...
class LRPViT:
    ...
    def predict(self, img):
        ...
        pred = self.model(img_tr.unsqueeze(0))
    ...
    def get_cam(self, img, class_index=0):
        ...
        transformer_attribution = self.attribution_generator.generate_LRP(
            # img.unsqueeze(0).cuda(), method="transformer_attribution", index=class_index
            img.unsqueeze(0),
        ...
        heatmap = cv2.applyColorMap(
            np.uint8(255 * transformer_attribution), cv2.COLORMAP_JET
        )
    ...
    def generate_visualization(self, original_image, class_index=None):
        ...
        image_transformer_attribution = (
            original_image.permute(1, 2, 0).data.cpu().numpy()
        )
        image_transformer_attribution = (
            image_transformer_attribution - image_transformer_attribution.min()
        ) / (image_transformer_attribution.max() - image_transformer_attribution.min())
        vis = self.show_cam_on_image(
            image_transformer_attribution, transformer_attribution
        )
